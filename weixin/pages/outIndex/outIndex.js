...
Page({
  ...
  outTimes: function () {
    ...
    wx.request({
      url: app.globalData.apiUrl + '/getAfterDetectionListByEmployeeId',
      method: "POST",
      header: {
        ...
        if ((returnData != null) && (returnData != [])) {
          if (returnData.length > 0) {
            ...
            for (let i = 0; i < returnData.length; i++) {
              dataLogs.push({ "date": returnData[i].date, "cylinderCount": returnData[i].cylinderCount, "batch": returnData[i].batch, "remark": returnData[i].remark });
              todayOutTimes += Number(returnData[i].cylinderCount);
            }

            that.setData({
              dataLogs: dataLogs,
              todayOutTimes: todayOutTimes
            })
          }
        }
      },
  ...
  addOut: function () {
    wx.navigateTo({
      url: '/pages/outCheckFull/outCheckFull'
    })
  }
